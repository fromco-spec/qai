    ...
    try:
        ...
        resp = client.messages.create(
            ...
            system=(
                ...
                "例）「スラヘルの成分を教えて」→「スラヘルの成分を教えて SLAHEL 原材料 原材料名 有効成分 機能性成分 配合 成分表\"\n"
                "例）「この商品の特徴は」→「この商品の特徴は 訴求ポイント 商品特徴 こだわり 効果 機能 説明\"\n"
                "余計な説明・記号・改行は不要。"
            ),
...
### ⑤ 商品案内の注意事項（よくある曖昧質問への対応）

**【シロッシュ（SIROSHU）に関する案内ルール】**

- **「研磨剤は入っていますか？」**
  → 「ヒドロキシアパタイトが研磨剤にあたります」と必ず答える。「入っていません」は誤りなので絶対に使わない。

- **「銀歯に使っていい？」**
  → 「ご使用いただいて問題はありません。ただし、長期的な使用や強く擦ることで表面が傷つく可能性があるため、気になる場合はかかりつけ医に原材料をお見せしてご相談ください」と案内する。

- **「アレルギーがあるけど使えますか？」**
  → まず「アレルギーがある場合は使用しないでください」と伝えた上で、「えび・かに・小麦・そば・卵・乳・落花生の7大アレルギーは問題ありません」と補足する。

- **「歯ブラシを濡らして使っていい？」**
  → 「乾いた歯ブラシでの使用が必須です。濡れた状態だと炭酸効果が薄まります」と案内する。

- **「薬用ですか？」「歯周病に効きますか？」「病気に効きますか？」**
  → 「シロッシュは薬用ではなく化粧品歯磨きです。歯周病などの病気に効くものではありません」と明確に伝える。

- **「カビが生えている」「白い塊・結晶がある」「固まっている」**
  → まず色を確認する。「白色であれば品質上の問題はなく、炭酸成分の結晶化です。よく振ってご使用ください。青色・黒色など色がついている場合は着払い返送と代品手配の対応になります」と案内する。

- **「炭酸歯磨きって何ですか？」「シュワシュワしますか？」**
  → 「口に含んだ瞬間に炭酸を感じますが、持続性はありません。重曹とクエン酸が水に溶けることで炭酸が発生し、歯の黄ばみ・汚れを浮かせます」と案内する。

- **「味が嫌い・しょっぱい」**
  → 「炭酸水素ナトリウム（重曹）の塩味です。市販の歯磨き粉の上にシロッシュをパラパラと乗せて一緒に使うと、味が気にならなかったとのお声もあります」と案内する。
